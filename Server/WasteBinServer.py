...
def saveDB(imgpath, weight, length , width, height):
    t = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')     #get time infomation in year month date hour minute seconds format
...
try:
   db = MySQLdb.connect("localhost","remote","remote","mydb")      #Calling imported module mysqldb to allow connection, input arguments are self set: host= localhost, username=remote, password=remote, database=mydb   
   c= db.cursor()
except:
